...
export function renderNewLessonDialog(dateKey, contextData = {}) {
  window.areaEl.innerHTML = `
    ...
    <div class="form-group" style="display: flex; flex-direction: column; gap: 10px; max-width: 400px;">
      ...
      <button id="create-lesson-btn" class="btn">Створити урок</button>
    </div>
  `;
  ...
  if (contextData?.preselectedClass) {
    classSelect.value = contextData.preselectedClass;
  }
  ...
  if (contextData?.preselectedSubject) {
    subjectSelect.value = contextData.preselectedSubject;
  }
  window.$("#create-lesson-btn").onclick = () => {
    const className = classSelect.value;
    const subjectName = subjectSelect.value;
    if (!className || !subjectName) {
      alert("Будь ласка, оберіть клас та предмет.");
      return;
    }
    const newLesson = {
      id: "lesson_" + Date.now(),
    ...
    studentsInClass.forEach(studentName => {
      newLesson.students[studentName] = {
        presence: true, work: false, hw: false, grade: "", note: ""
      };
    });
...
export function renderLesson(id){
  ...
  studentsInClass.forEach(name => {
    if (!lessonStudents[name]) {
      lessonStudents[name] = { presence: true, work: false, hw: false, grade: "", note: "" };
    }
  });
  Object.keys(lessonStudents).forEach(name => {
    if (!studentsInClass.includes(name)) {
      delete lessonStudents[name];
    }
  });
  les.students = lessonStudents;
  window.areaEl.innerHTML = `
    <div class="lesson-header">
      <input class="input" id="l-date" type="date" value="${les.date.split('T')[0]}" style="width: 150px;">
      <select class="input" id="l-class" style="width: 150px;"></select>
      <select class="input" id="l-subj" style="width: 150px;"></select>
      <input class="input title-input" id="l-title" placeholder="Тема уроку" value="${window.esc(les.title)}">
      
      <button class="btn" id="l-createboard">🎨 Створити дошку</button>
      <button class="btn" id="l-addfiles">+ Файли</button>
      </div>
  ...
  window.$("#l-createboard").onclick = async () => {
    ...
    if (newBoard) {
      les.files = (les.files || []);
      les.files.push({
        name: newBoard.filePath.split('\\').pop(), // Використовуємо єдине поле 'name'
        saved_path: newBoard.filePath,
        type: 'board'
      });
      window.saveLessons();
      window.renderLesson(id);
  ...
  sortedStudentNames.forEach(studentName => {
    ...
    tr.innerHTML = `
      ...
      <td style="text-align: center;"><input type="checkbox" ${s.presence?"checked":""}></td>
      <td style="text-align: center;"><input type="checkbox" ${s.work?"checked":""}></td>
      <td style="text-align: center;"><input type="checkbox" ${s.hw?"checked":""}></td>
      <td><select class="input grade-input" style="width: 70px;">${gradeOptions}</select></td>
      <td><input class="input" value="${window.esc(s.note||"")}" placeholder="..."></td>`;
    const [pres, work, hw] = window.$$("input[type=checkbox]", tr);
    const gradeInput = window.$("select.grade-input", tr); 
    const noteInput = window.$("input.input:not(.grade-input)", tr);
    const styleGrade = (selectElement) => {
      const val = parseInt(selectElement.value, 10);
      selectElement.className = "input grade-input"; 
      selectElement.style.width = "70px"; 
    ...
    pres.onchange = () => { s.presence = pres.checked; window.saveLessons(); };
    work.onchange = () => { s.work = work.checked; window.saveLessons(); };
    hw.onchange   = () => { s.hw = hw.checked; window.saveLessons(); };
    gradeInput.onchange = () => { 
      s.grade = gradeInput.value; 
  ...
  if (les.files && les.files.length) { 
    ...
    les.files.forEach((f,i)=>{
      const tr = document.createElement("tr");
      // === ВИПРАВЛЕННЯ: Використовуємо name АБО original_name для сумісності ===
      ...
      tr.innerHTML = `<td>${icon} ${window.esc(fileName)}</td>
        <td><div style="display: flex; gap: 8px;"><button class="btn open">Відкрити</button> <button class="btn danger del">Видалити</button></div></td>`;
      
      window.$(".open",tr).onclick = ()=> {
        if (isBoard) {
          const boardExists = window.state.boards.find(b => b.filePath === f.saved_path);
          if (boardExists) {
            window.tj.openBoardWindow(f.saved_path);
          } else {
            window.showCustomAlert("Помилка", "Файл дошки існує, але запис про нього відсутній у базі.");
          }
        } else {
...
export function renderLessonsList(){
  window.areaEl.innerHTML = `
    ...
    <div class="lesson-filters">
      ...
      <div class="lesson-filters-buttons">
        <button id="create-lesson-quick-btn" class="btn">Створити урок</button>
  ...
  function populateList() {
    ...
    t.className = "table";
    t.innerHTML = "<thead><tr><th>Дата</th><th>Тема</th><th>Клас</th><th>Предмет</th><th style='width: 220px;'>Дії</th></tr></thead><tbody></tbody>";
      ...
      tr.innerHTML = `
        ...
        <td>
          <div style="display: flex; gap: 8px;">
            <button class='btn btn-open-lesson'>Відкрити</button>
            <button class='btn danger btn-del-lesson'>Видалити</button>
          </div>
        </td>`;
...
export async function showLessonListContextMenu(e, lessonId, lessonTitle, lessonDate, autoDelete = false) {
  ...
  if (autoDelete) {
    ...
    const menuItems = [
      { label: "Видалити урок...", click: deleteAction }
    ];
